...
function lidar_update_3d(name, imagedata, obj) {
    ...
    // Populate Image
    for (var row2 = 0; row2 < obj.depth.length; row2++) {
      for (var col2 = 0; col2 < obj.depth[row2].length; col2++) {
        var val2 = obj.depth[row2][col2];
        var color;
        if (is_valid(val2)) {
          var idx = parseInt(spectrum_length * (val2 - min_value) / (max_value - min_value));
          color = spectrum[idx];
        } else if (val2 === 65400 || val2 === 65500) {
          color = white;
        } else {
          color = black;
        }
        big_point(name, imagedata, col2, row2, color);
      }
    }
...
function lidar_update(name, imagedata, obj) {
    ...
    // Center point
    var cx = x_origin * canvas.width;
    var cy = y_origin * canvas.height;
    ...
    for (var i = 0; i < obj.scan.length; i++) {
      var a = obj.scan[i][0] * deg_to_rad;
      var d = obj.scan[i][1] * scale;
  
      var x = d * Math.sin(a) + x_origin * canvas.width;
      var y = -d * Math.cos(a) + y_origin * canvas.height;
  
      var dx = last_x - x;
      var dy = last_y - y;
      var distance = Math.sqrt(dx * dx + dy * dy);
  
      // If close enough, connect with line
      if (distance < d * 0.05) {
...
export default function Image() {  
    return (
      <div style={{ textAlign: 'center' }}>
        {/* These <div> and <canvas> IDs match the references in the code above */}
        <div id="msg" style={{ margin: '1rem', color: 'red' }}>&nbsp;</div>
        <div id="lidar_status" style={{ marginBottom: '0.5rem' }}>OFF</div>
